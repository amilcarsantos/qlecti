...
function testArrays() {
  ...
  QUnit.test('first', function(assert) {
    ...
    //NEVER assert.equal(_([1, 2, 3]).first(), 1, 'can perform OO-style "first()"');
    ...
    //TODO:nulls.js// assert.equal(_.first(null), void 0, 'handles nulls');
...
*/
  ...
  QUnit.test('empty', function(assert) {
    promiseValue = '_xxx_';
    assert.equal(_promise(Ql.on([]).empty(function() {
      promiseValue = '_is_empty_';
    })), '_is_empty_', 'can notify an empty array');
    promiseValue = '_not_called_';
    assert.equal(_promise(Ql.on([1]).empty(function() {
        promiseValue = 'is_empty';
    })), '_not_called_', 'can skip an non empty array');
  });
...
*/ 
  ...
  QUnit.test('without', function(assert) {
    ...
    assert.deepEqual(Ql.on(list).op().without(0, 1)
		.ret().val(), [2, 3, 4], 'can remove all instances of an object');
/*  TODO  var result = (function(){ return _.without(arguments, 0, 1); }(1, 2, 1, 0, 3, 1, 4));
    assert.deepEqual(result, [2, 3, 4], 'works on an arguments object');*/

    list = [{one: 1}, {two: 2}];
    assert.equal(Ql.on(list).op().without({one: 1})
		.ret().val().length, 2, 'uses real object identity for comparisons.');
    assert.equal(Ql.on(list).op().without(list[0])
		.ret().val().length, 1, 'ditto.');
  });
...
/** TODO: .op().unique() 
    <tt> .op().unique('subkey') </tt>  ou
	<tt> .op().subKey('subkey').unique() </tt>
